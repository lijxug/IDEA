...
for(target_CDType in target_CDTypes) {
  ...
  if (target_CDType == "CD8") {
    chn_clusters = all_sceObj$characteristics..majorCluster %>% unique() %>% grep(pattern = target_CDType, value = T) %>% 
      grep(pattern = "C7", invert = T, value = T) # extract CD8+ T cells without MAIT cells 
  ...
  write_rds(usageMat, path = paste0(out_dir, "/", target_CDType, "_flt_usageMat.rds"))
