...
# out_dir = "/your/path/"
input_path = paste0(wd, "/../IDEA_data/06.data_totry/p01.scLUNGsceset.RDS_2018July18") 
out_dir = "/data2/jason/data/IDEA_data/03.IDEAoutputs/"
...
chn_clusters = sceObj$characteristics..majorCluster %>% unique() %>% grep(pattern = target_CDType, value = T) %>% grep(pattern = "C7", invert = T, value = T) # extract CD8+ T cells without MAIT cells
...
```{r}
wb = EXWB.initiate()
wb = EXWB.writeSheet(wb, sheetName = paste0("enrichment_", target_CDType), sheetData = enrichment_tbl)
